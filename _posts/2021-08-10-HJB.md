...
This is the discrete cousin of HJB. The state $s$ corresponds to $x$, the action $a$ corresponds to $u$, and the transition probabilities $P(s'|s,a)$ correspond to the dynamics.

For a cost-minimization problem, the same equation is often written as

$$
V^*(s)=\min_a \left[
c(s,a) + \gamma \sum_{s'} P(s'|s,a)V^*(s')
\right].
$$
